...
- [ ] **Step 1: Write failing tests** for the exact `LOGS_PATH`/`EXEC_PATH`, missing/wrong/correct token in the mandatory first `Hello` frame before ticket redemption, single-use bound tickets, selected container, tail/since/follow/pause/find, truncation, explicit shell connect, stdin/stdout/stderr/resize/exit, RBAC/missing-binary errors, and terminal disconnect on socket loss.
