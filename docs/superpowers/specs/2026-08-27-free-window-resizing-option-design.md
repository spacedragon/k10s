...
- Windows remain constrained to the workspace canvas in both modes.
...
- Normal: existing kind-specific minimum size, no window-level scrolling added by this feature.
...
The preference is part of `WorkspaceSnapshot`, because it controls persisted workspace layout behavior. The current snapshot schema is extended through a new version. Loading older supported snapshots migrates the missing preference to `false`; new snapshots always write the explicit boolean.
...
7. Older supported snapshots migrate with the setting off.
8. Accessibility snapshots are regenerated only where the checked menu item or free-mode scroll container intentionally changes structure.
